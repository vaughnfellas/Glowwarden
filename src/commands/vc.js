...
import { canUseTempVCCommands, getUserOwnedTempVC } from '../utils/owner.js';
...
export async function execute(interaction) {
  try {
    ...
    if (subcommand === 'goto') {
      ...
      // Check permissions using centralized utils
      const permissionCheck = canUseTempVCCommands(member);
      ...
      try {
        ...
        for (const [channelId, channel] of tempVCs) {
          ...
          try {
            ...
            console.warn(`Failed to fetch owner ${ownerId} for channel ${channelId}:`, error);
            continue;
          }
        }
        ...
        try {
          ...
          console.error(`Failed to move ${member.user.tag} to War Chamber:`, error);
          return interaction.reply({
      ...
      try {
        ...
        // Check if the command user has a War Chamber using centralized utils
        const userChannelId = getUserOwnedTempVC(interaction.user.id);
        if (!userChannelId) {
          return interaction.reply({
        ...
        if (!userChannel) {
          return interaction.reply({
            content: 'Your War Chamber could not be found. Please create a new one.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (result.success) {
          ...
          try {
            await targetMember.send(
              `**${interaction.user.username}** has invited you to their War Chamber! ` +
              `You now have access to join **${userChannel.name}**.`
            );
          } catch (error) {
            console.log(`Could not DM invite to ${targetMember.user.tag}:`, error);
          }
        ...
        console.error('VC invite command error:', error);
        return interaction.reply({
          content: 'Something went wrong while processing the invite. Please try again.',
          flags: MessageFlags.Ephemeral
        });
      }
    }
  ...
  try {
    ...
    for (const [channelId, channel] of tempVCs) {
      ...
      try {
        ...
        console.warn(`Failed to fetch owner ${ownerId} for autocomplete:`, error);
        continue;
      }
    }
